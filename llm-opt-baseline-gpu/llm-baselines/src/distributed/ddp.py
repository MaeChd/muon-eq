...
class DataParallelDistributedBackend(DistributedBackend):
    ...
    def get_adjusted_args_for_process(self, args):
        ...
        if args.acc_steps < 1:
            raise ValueError(f"acc_steps must be >= 1, got {args.acc_steps}.")

        # Keep the user-facing semantics conventional in DDP:
        # - batch_size is the per-rank micro-batch size
        # - acc_steps is the number of microsteps to accumulate before optimizer.step()
