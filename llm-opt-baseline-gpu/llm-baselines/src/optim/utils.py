...
def _summarize_invalid_tokens(tensor, vocab_size, allow_ignore_index=False, limit=8):
    invalid_mask = tensor < 0
    if allow_ignore_index:
        invalid_mask = invalid_mask & (tensor != -1)
    invalid_mask = invalid_mask | (tensor >= vocab_size)
    if not torch.any(invalid_mask).item():
        return []

    invalid_values = torch.unique(tensor[invalid_mask])
    return invalid_values[:limit].tolist()


def _validate_batch_token_range(x, y, vocab_size, batch_name="batch"):
    ...
    x_ok = x_min >= 0 and x_max < vocab_size
    y_ok = y_min >= -1 and y_max < vocab_size
    if x_ok and y_ok:
        return

    bad_x = _summarize_invalid_tokens(x, vocab_size, allow_ignore_index=False)
    bad_y = _summarize_invalid_tokens(y, vocab_size, allow_ignore_index=True)
    raise ValueError(
        f"Token ids out of range for vocab_size={vocab_size} in {batch_name}.\n"
        f"x: shape={tuple(x.shape)} dtype={x.dtype} range=[{x_min}, {x_max}] invalid={bad_x}\n"
        f"y: shape={tuple(y.shape)} dtype={y.dtype} range=[{y_min}, {y_max}] invalid={bad_y}\n"
        "Likely causes:\n"
        "  1. dataset tokenizer does not match the model vocab\n"
        "  2. .bin shards are being read with the wrong dtype\n"
        "  3. dataset files are not the expected fineweb GPT-2 token bins"
    )


def get_batch(datareader, device="cpu", vocab_size=None, batch_name="batch"):
    x, y = datareader.sample_batch()
    _validate_batch_token_range(x, y, vocab_size=vocab_size, batch_name=batch_name)
