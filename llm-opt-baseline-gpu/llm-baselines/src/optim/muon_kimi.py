...
def canonicalize_zeropower_mode(zeropower_mode):
    canonical_mode = ZEROPOWER_MODE_ALIASES.get(str(zeropower_mode).lower())
    if canonical_mode is None:
        raise ValueError(
            f"Unsupported MuonKimi zeropower_mode={zeropower_mode!r}. "
            "Expected one of: native, spc."
        )
    return canonical_mode


def apply_zeropower_backend(g, steps, zeropower_mode):
...
class Muon(torch.optim.Optimizer):
    ...
    Arguments:
        ...
        zeropower_mode: Which zeropower backend to use. One of {"native", "spc"}.
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                u = apply_zeropower_backend(
                    ...
                    zeropower_mode=group.get("zeropower_mode", "native"),
                )
