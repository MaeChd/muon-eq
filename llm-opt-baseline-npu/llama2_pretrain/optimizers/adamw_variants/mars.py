...
def update_fn(p, grad, exp_avg, exp_avg_sq, lr, wd, beta1, beta2, last_grad, eps, amsgrad, max_exp_avg_sq, step, gamma,
              mars_type, is_grad_2d, optimize_1d, lr_1d_factor, betas_1d, weight_decay_1d,mgup,mgup_alpha,mgup_gamma,norm_type,cwd):
    ...
    if optimize_1d or is_grad_2d:
        ...
        if norm_type == 'col':
            # Column-wise normalization: divide each column by its norm.
            c_t_norm = torch.norm(c_t, dim=0, keepdim=True)  # shape: (1, n_cols)
            c_t = c_t / (c_t_norm + 1e-8)  # Add a small constant to avoid division by zero.
        elif norm_type == 'row':
            # Row-wise normalization: divide each row by its norm.
            c_t_norm = torch.norm(c_t, dim=1, keepdim=True)  # shape: (n_rows, 1)
            c_t = c_t / (c_t_norm + 1e-8)
        elif norm_type == 'all':
            # Global normalization with the Frobenius norm.
            c_t_norm = torch.norm(c_t)
            if c_t_norm > 1.:
                c_t = c_t / c_t_norm
        if mars_type == "mars-adamw-v":
            exp_avg.mul_(beta1).add_(grad, alpha=1. - beta1)
        else:
            exp_avg.mul_(beta1).add_(c_t, alpha=1. - beta1)
        
        if mgup:
            alignment = (exp_avg * c_t >= 0).float()
            # scale = alignment.mean().clamp(min=1e-2)
            mask = alignment * mgup_alpha + (1.0 - alignment) * mgup_gamma
        if cwd:
            cwd_mask = (exp_avg * p.data >0).float()
        else:
            cwd_mask = 1.0
        if (mars_type == "mars-adamw") or (mars_type == "mars-adamw-v") or (mars_type == "mars-shampoo" and not is_grad_2d):
            ...
            real_update_tmp = -lr * torch.mul(p.data * cwd_mask, wd).add(exp_avg.div(denom))
        elif mars_type == "mars-adamw-m":
            ...
            real_update_tmp = -lr * torch.mul(p.data * cwd_mask, wd).add(exp_avg.sign())
        ...
        if mgup:
            p.data.add_(real_update_tmp * mask)
        else:
        ...
        if mgup:
            alignment = (exp_avg * grad >= 0).float()
            # scale = alignment.mean().clamp(min=1e-2)
            mask = alignment * mgup_alpha + (1.0 - alignment) * mgup_gamma
        if cwd:
            cwd_mask = (exp_avg * p.data >0).float()
        ...
        if amsgrad:
            ...
            denom = exp_avg_sq.sqrt().mul(1/math.sqrt(bias_correction2)).add(eps).mul(bias_correction1)
        real_update_tmp = -lr * lr_1d_factor * torch.mul(p.data * cwd, weight_decay_1d).add(exp_avg.div(denom))
        # p.data.add_(real_update_tmp)
        if mgup:
            p.data.add_(real_update_tmp * mask)
        else:
...
class MARS(Optimizer):
    def __init__(self, params, lr=3e-3, betas=(0.95, 0.99), eps=1e-8, weight_decay=0., amsgrad=False, gamma=0.025, 
                 ...
                 mgup = False , mgup_alpha = 2.0,mgup_gamma=0.1,norm_type='all',cwd=False):
        if not 0.0 <= lr:
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        ...
                        mgup=mgup,mgup_alpha=mgup_alpha,mgup_gamma=mgup_gamma,norm_type=norm_type)
        ...
        for group in self.param_groups:
            for p in filter(lambda p: exists(p.grad), group['params']):
                ...
                exp_avg, exp_avg_sq = self.update_fn(
                    ...
                    mgup=self.mgup,
                    mgup_alpha=self.mgup_alpha,
                    mgup_gamma=self.mgup_gamma,
