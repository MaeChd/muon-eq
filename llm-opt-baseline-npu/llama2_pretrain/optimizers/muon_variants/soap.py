...
class SOAP(optim.Optimizer):
    ...
    def update_preconditioner(self, grad, state, 
        ...
        # Ensure we use the correct dtype for the update (usually float32 for optimizer states)
        target_dtype = torch.float32

        if grad.dim() == 1:
            if precondition_1d and grad.shape[0] <= max_precond_dim:
                # FIX: Calculate update and cast to float32 before lerp
                update_term = (grad.unsqueeze(1) @ grad.unsqueeze(0)).to(dtype=target_dtype)
                state['GG'][0].lerp_(update_term, 1-state['shampoo_beta'])
        else:
            if merge_dims:
                new_grad = self.merge_dims(grad, max_precond_dim)
                for idx, sh in enumerate(new_grad.shape):
                    if sh <= max_precond_dim:
                        ...
                        # FIX: Cast outer_product to float32 before lerp
                        state['GG'][idx].lerp_(outer_product.to(dtype=target_dtype), 1-state['shampoo_beta'])
            else:
                for idx, sh in enumerate(grad.shape):
                    if sh <= max_precond_dim:
                        outer_product = torch.tensordot(
